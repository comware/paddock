...
export function BottomNav() {
   ...
   * growing space - so by the time the page renders the URL is /microgreens/site/1/trays and a
   * prefix match against /microgreens/trays finds nothing. Comparing the last segment keeps the
  ...
  const isActive = (path: string) => {
    ...
    if (path === '/microgreens') {
      // Only the growing space dashboard itself, not everything beneath it.
      return current === '/microgreens' || /^\/microgreens\/site\/[^/]+$/.test(current);
    }
