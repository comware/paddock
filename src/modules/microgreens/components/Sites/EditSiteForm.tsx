...
export function EditSiteForm({ isOpen, onClose, site }: EditSiteFormProps) {
  ...
  const handleDetectLocation = async () => {
    ...
    try {
      ...
      try {
        ...
        if (detectedAddress) {
          setValue('address', detectedAddress, { shouldDirty: true });
        }
      } catch {
  ...
  const handleGeocodeAddress = async () => {
    ...
    try {
      ...
      if (result) {
        setValue('latitude', Math.round(result.lat * 10000) / 10000, { shouldDirty: true });
        setValue('longitude', Math.round(result.lng * 10000) / 10000, { shouldDirty: true });
        // Update address with the standardized display name
        setValue('address', result.displayName, { shouldDirty: true });
      } else {
