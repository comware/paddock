...
export function useTrayMigration(): MigrationStatus {
  ...
  useEffect(() => {
    ...
    // Already complete or running
    if (status.isComplete || status.isRunning) return;
    ...
    runMigration();
  }, [traysLoading, sitesLoading, trays, sites, status.isComplete, status.isRunning, ensureDefaultSite, migrateOrphanTrays]);
